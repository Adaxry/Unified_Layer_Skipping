...
class ModelArguments:
    ...
    freeze_emb: bool = field(
        default=True,
        metadata={"help": "whether freeze the weights of embeddings"},
    )
    ...
    target_speedup: Optional[int] = field(
        default=None,
        metadata={"help": "target_speedup used for layer skipping, e.g., given a 30-layer model and 5x target_speedup,"
                          " it will select 6 layers for output predictions"},
    )
...
def main():
    ...
    if model_args.model_name_or_path:
        ...
        # set layers to keep when conduct generation according to target_speedup
        if model_args.target_speedup is not None:
            num_total_model_layer = config.n_layer
            
            skip_decode_reserved_layers = [0, num_total_model_layer-1] # always reserve the first and final layer
            
            num_reserved_layer = int(num_total_model_layer / model_args.target_speedup)
            interval = int(num_total_model_layer / (num_reserved_layer - len(skip_decode_reserved_layers)+1))
            
            for idx in range(1, num_total_model_layer-1):
                if idx % interval == 0 and len(skip_decode_reserved_layers) < num_reserved_layer:
                    skip_decode_reserved_layers.append(idx)
            
            skip_decode_reserved_layers.sort()
            model.skip_decode_reserved_layers = skip_decode_reserved_layers
    ...
    def preprocess_function(examples):
        with CaptureLogger(tok_logger) as cl:
            ...
            if "prefix" in column_names:
                ...
                # record the prefix_lengths, for post training of skip decode
                text_tokenized["prefix_lengths"] = prefix_lengths 
