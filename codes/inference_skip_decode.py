...
if __name__ == "__main__":
    ...
    parser.add_argument('--target_speedup', type=int, default=None, help='set the same value as the training stage')
    parser.add_argument('--output-consume-time-for-decoding', action='store_true')
    ...
    skip_decode_allowed_layers = args.skip_decode_allowed_layers
    model = AutoModelForCausalLM.from_pretrained(model_name_or_path, torch_dtype=torch.float16, device_map="auto")
    print(model.hf_device_map)
    model.skip_decode_allowed_layers = skip_decode_allowed_layers
    model.output_consume_time_for_decoding = args.output_consume_time_for_decoding
